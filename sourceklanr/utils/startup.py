...
LOGS = logging.getLogger("تليثون كاتيا \n ")
...
async def startupmessage():
    try:
        if BOTLOG:
            Config.CATUBLOGO = await iqthon.tgbot.send_file(BOTLOG_CHATID, "https://graph.org/file/b66ef25850f144ce1ce46.jpg", caption="**⁂ - تـمّ  اعـادة تشـغيل .\n⁂ - تليثـون كاتيا ( 8.3 ) .\n\n⁂ - اوامر السورس : ( .الاوامر  ) \n\n⁂ - لمـعرفة كيفية تغير بعض كلايش او صور السـورس  أرسـل  : (  .مساعده  )\n\n⁂ - القناة تليثون كاتيا : @Source_Katya \n\n❕- يتم اعادة التشغيل كل 24 ساعة ⁂**",                buttons=[(Button.url("مطور تليثون الرسمي", "https://t.me/ZzZzZl1l"),)],            )
    except Exception as e:
        LOGS.error(e)
        return None
...
async def iqchn():
    ...
    try:
        ...
        await iqthon(JoinChannelRequest("@Source_Katya"))
    except BaseException:
        pass
